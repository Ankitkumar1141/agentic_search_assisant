...
.report-panel { border: 1px solid rgba(255,140,50,0.2); }
.feedback-panel { border: 1px solid rgba(80,200,120,0.2); }
        ...
        if st.session_state.running:
            for key in steps:
                if key not in results:
                    return "running" if key == step else "waiting"
        return "waiting"

    step_card("01", "Search Agent", step_state("search"), "Gathers recent web information")
    step_card("02", "Reader Agent", step_state("reader"), "Scrapes and extracts deep content")
    step_card("03", "Writer Agent", step_state("writer"), "Drafts the full research report")
    step_card("04", "Critic Agent", step_state("critic"), "Reviews and scores the report")
...
if st.session_state.running and not st.session_state.done:
    ...
    with st.spinner("🔍  Search Agent is working…"):
        pipeline_results["search"] = run_search_agent(topic_val)
        st.session_state.results = dict(pipeline_results)

    with st.spinner("📄  Reader Agent is scraping top resources…"):
        pipeline_results["reader"] = run_reader_agent(
            topic_val, pipeline_results["search"]
        )
        st.session_state.results = dict(pipeline_results)
    ...
    with st.spinner("✍️  Writer is drafting the report…"):
        ...
        pipeline_results["writer"] = run_writer_agent(topic_val, research_combined)
        st.session_state.results = dict(pipeline_results)

    with st.spinner("🧐  Critic is reviewing the report…"):
        pipeline_results["critic"] = run_critic_agent(pipeline_results["writer"])
        st.session_state.results = dict(pipeline_results)
...
st.markdown(
    """
<div class="notice">
    ResearchMind · Powered by Mistral AI multi-agent pipeline · Built with Streamlit
</div>
""",
    unsafe_allow_html=True,
)
